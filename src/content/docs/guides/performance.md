---
slug: performance
title: High Performance Execution & Multi-Tier CodeGen
sidebar:
  order: 1
---

Jaci is designed for blazing-fast execution in general-purpose software, numerical computing, and native embedding. High performance in Jaci is achieved across two major execution tiers:

1. **Optimized VM & Bytecode Loop**: A compact, instruction-cache-efficient C-based virtual machine loop with direct dispatch and fast builtin intrinsics.
2. **Multi-Tier Native Optimization Pipeline**: An optimizing multi-tier compilation architecture: `IR -> HIR -> MIR -> LLVM IR -> Optimized Native Code`

## Multi-Tier Optimization Architecture

As documented in [ADR 0006](https://github.com/jaci-lang/jaci/blob/master/docs/adr/0006-multi-tier-optimization-architecture.md) and [ADR 0007](https://github.com/jaci-lang/jaci/blob/master/docs/adr/0007-llvm-backend-and-table-optimization.md):
...
- **HIR (High-Level Intermediate Representation)**:
  ...
  - Scalar Replacement of Aggregates (SROA) for non-escaping virtual tables and closures.

- **MIR (Medium-Level Intermediate Representation)**:
  - Concrete SSA machine representation using unboxed representation-level types (`Int32`, `Int64`, `Float64`, `Bool`, `Tag`, `RawPtr`, `GcPtr`, `ManagedRef`, `TValueBoxed`).
  - Redundant Guard Elimination, Bounds Check Elimination, GVN/CSE, LICM, Load/Store Elimination, and GC Write Barrier Elimination.

- **LLVM Backend & Native FFI Dispatch**:
  - Emits optimized LLVM IR leveraging whole-function SIMD auto-vectorization, global instruction scheduling, and advanced register allocation.
  - Direct native C ABI dispatch (`call ccc`) for Foreign Function Interface (`ffi.C`) calls without trampoline overhead.
