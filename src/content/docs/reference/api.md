...
slug: api
title: C API
description: The official reference for Jaci's C API and embedder interfaces.
sidebar:
  order: 3
---

The Jaci Virtual Machine exposes a C API that allows embedders to manipulate it, manage runtime state, and execute code.
It is declared through the [`VM/include`](https://github.com/jaci-lang/jaci/tree/master/VM/include) header files:

* `lua.h` - Primary VM APIs, members use either the `lua_` or `luau_` prefix
* `lualib.h` - Helpers and standard library modules (`luaopen_fs`, `luaopen_ffi`, `luaopen_json`, `luaopen_net`, etc.)
